...
class ExerciseContentGenerator {
  private static readonly EXERCISE_TEMPLATES = {
    // Core & Planks
    'plank': {
      setup: [
   ...
   */
  static generateExerciseContent(exercise: any): {
    description: string;
    setup: string[];
    execution: string[];
    breathing: any;
  ...
  private static generateDescription(exercise: any, type: string): string {
 ...
 */
function convertDifficulty(difficulty: string): "Beginner" | "Intermediate" | "Advanced" {
  const difficultyLower = difficulty.toLowerCase();
  if (difficultyLower.includes('advanced') || difficultyLower.includes('expert')) return "Advanced";
  if (difficultyLower.includes('intermediate') || difficultyLower.includes('moderate')) return "Intermediate";
  return "Beginner";
}

async function migrateEnhancedExerciseDatabase(): Promise<void> {
  ...
  try {
    ...
    console.log(`Found ${exerciseDatabase.exercises.length} exercises to migrate with enhanced details...`);
...
export { migrateEnhancedExerciseDatabase };