...
import { Database, X, Filter, Check, RotateCcw, Search as SearchIcon, AlertTriangle } from 'lucide-react';
...
const tier2Categories: { label: string; value: string | 'All' }[] = [
  { label: 'All', value: 'All' },
  { label: 'Strength', value: 'Strength' },
  { label: 'Cardio', value: 'Cardio' },
  { label: 'Mobility & Flexibility', value: 'Mobility & Flexibility' },
  { label: 'Balance & Stability', value: 'Balance & Stability' },
  { label: 'Power & Explosiveness', value: 'Power & Explosiveness' },
];

const difficultyLevels: { label: string; value: string | 'All' }[] = [
  { label: 'All', value: 'All' },
...
const ExerciseLibraryScreen: React.FC = () => {
  ...
  const [selectedTier2, setSelectedTier2] = useState<string | 'All'>('All');
  const [selectedDifficulty, setSelectedDifficulty] = useState<string | 'All'>('All');
  const [searchQuery, setSearchQuery] = useState('');
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [problematicExercises, setProblematicExercises] = useState<string[]>([]);
  ...
  // Error boundary component for individual exercises
  const ExerciseCard: React.FC<{ exercise: CatalogExercise }> = ({ exercise }) => {
    const [hasError, setHasError] = React.useState(false);
    
    if (hasError) {
      return (
        <div className="group flex items-center gap-4 bg-white dark:bg-gray-800 p-3 rounded-[28px] shadow-sm border border-slate-100 dark:border-gray-700 transition-all hover:shadow-md cursor-pointer active:scale-[0.98]">
          <div className="shrink-0">
            <div className="bg-slate-50 dark:bg-gray-700 rounded-2xl w-24 h-24 border border-slate-50 dark:border-gray-600 flex items-center justify-center">
              <span className="material-symbols-outlined text-slate-300 dark:text-slate-500 text-2xl">error</span>
            </div>
          </div>
          <div className="flex flex-1 flex-col justify-between h-24 py-1">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-slate-900 dark:text-gray-100 text-lg font-black leading-tight line-clamp-1 tracking-tight">{exercise.name || 'Unknown Exercise'}</h3>
                <p className="text-amber-500 text-[10px] font-black uppercase tracking-[0.2em] mt-1 opacity-90">Data Issue Detected</p>
              </div>
              <button className="text-slate-200 dark:text-gray-700 hover:text-red-500 transition-colors">
                <span className="material-symbols-outlined leading-none text-2xl" style={{ fontVariationSettings: "'FILL' 0" }}>favorite</span>
              </button>
            </div>
            <div className="flex flex-wrap gap-1 mt-auto">
              <span className="inline-flex items-center px-2 py-0.5 rounded-lg bg-amber-50 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 text-[10px] font-black tracking-widest uppercase border border-amber-100 dark:border-amber-500/30">
                Report Issue
              </span>
            </div>
          </div>
    ...
    try {
      ...
      const hasCriticalData = exercise.name && exercise.tier_2;
      const hasMetricData = exercise.metric?.type;
      const hasTierData = exercise.tier_1 && exercise.tier_2;

      if (!hasCriticalData || !hasMetricData || !hasTierData) {
        if (!problematicExercises.includes(exercise.id)) {
          setProblematicExercises(prev => [...prev, exercise.id]);
        }
      }
        ...
        >
          <div className="shrink-0">
            <div 
              ...
              style={{ backgroundImage: `url(${exercise.media?.imageUrl || 'https://images.unsplash.com/photo-1517836357463-d25dfeac3438?auto=format&fit=crop&q=80&w=200'})` }}
            ></div>
          </div>
          <div className="flex flex-1 flex-col justify-between h-24 py-1">
            <div className="flex justify-between items-start">
              <div>
                <h3 className="text-slate-900 dark:text-gray-100 text-lg font-black leading-tight line-clamp-1 tracking-tight">
                  {exercise.name || 'Unnamed Exercise'}
                </h3>
                <p className="text-primary text-[10px] font-black uppercase tracking-[0.2em] mt-1 opacity-90">
                  {exercise.tier_2 || 'Uncategorized'}
                </p>
              </div>
            ...
            <div className="flex flex-wrap gap-1 mt-auto">
              <span className="inline-flex items-center px-2 py-0.5 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400 text-[10px] font-black tracking-widest uppercase border border-slate-100 dark:border-gray-600">
                {exercise.metric?.type || 'Not specified'}
              </span>
              <span className="inline-flex items-center px-2 py-0.5 rounded-lg bg-slate-50 dark:bg-slate-700 text-slate-500 dark:text-slate-400 text-[10px] font-black tracking-widest uppercase border border-slate-100 dark:border-gray-600">
      ...
      console.error('Exercise card render error:', error);
      setHasError(true);
      return null;
    }
  };

  // Enhanced pagination with server-side tier filtering
  const { data: paginatedExercises, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useInfiniteQuery({
    queryKey: ['catalogExercises', selectedCategory, searchQuery],
    queryFn: async ({ pageParam }) => {
      // Build query based on selected tier
      let q = query(
      ...
      // Apply tier filtering if not "All"
      if (selectedCategory !== 'All') {
        q = query(q, where('tier_1', '==', selectedCategory));
      }
      ...
      const docs = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as CatalogExercise));
  ...
  // Extract all loaded exercises
  const allExercises = paginatedExercises?.pages.flatMap(page => page.exercises) || [];
  ...
  const availableFilters = useMemo(() => {
    if (!allExercises) return { tags: [], equipment: [] };
    const tags = new Set<string>();
    const equipment = new Set<string>();

    allExercises.forEach(ex => {
      ex.musclesTargeted?.forEach(t => tags.add(t));
  ...
  const filteredExercises = useMemo(() => {
    ...
    return searchResults.filter(ex => {
      ...
      return matchesTags && matchesEquipment;
    });
  }, [searchResults, selectedTags, selectedEquipment]);
  ...
  const handleSeed = async () => {
    if (confirm("Seed full production exercise catalog into Firestore?")) {
      ...
      // Note: In a real implementation, you'd use queryClient.invalidateQueries
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-background-dark pb-32">
      {/* Use mobile container for consistent spacing */}
      <div className="mobile-container space-y-6">
        {/* Header Section */}
        <header className="sticky top-0 z-20 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-md pt-6 pb-2 border-b border-primary/5">
          ...
          {isLoading ? (
            ...
            <div className="text-center py-24 text-slate-400 space-y-4">
              ...
              <p className="font-black uppercase tracking-widest text-sm">
                {searchQuery ? 'No matches found' : 'No exercises available'}
              </p>
              {(searchQuery || selectedCategory !== 'All' || selectedTags.length > 0 || selectedEquipment.length > 0) && (
            ...
            <>
              ...
              {hasNextPage && (
                <div ref={loadMoreRef} className="flex justify-center py-6">
                  <div className="text-center text-slate-400">
                    ...
                    <p className="text-xs font-black uppercase tracking-widest">Loading more exercises...</p>
                  </div>
                </div>
              )}
              ...
              {!hasNextPage && allExercises.length > 0 && (
                <div className="text-center py-6 text-slate-400">
                  <p className="text-xs font-black uppercase tracking-widest">End of exercises</p>
                  <p className="text-[10px] font-medium mt-1">Showing {allExercises.length} exercises</p>
                </div>
              )}
            </>
          )}
